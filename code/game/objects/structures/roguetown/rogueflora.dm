...
/obj/structure/flora/roguegrass/thorn_bush/attack_hand(mob/living/user)
	...
	user.visible_message("<span class='info'>[user] searches through [src].</span>")
	...
	if(do_after(L, rand(5,20), target = src))
		...
		if(prob(prob2findstuff))
			...
			to_chat(user, "<span class='warning'>You cut yourself on the thorns!</span>")
	...
	if(isliving(AM))
		...
		if(L.m_intent == MOVE_INTENT_WALK)
			if(!ishuman(L))
				...
				to_chat(L, "<span class='warning'>I'm scratched by the thorns.</span>")
				L.apply_damage(5, BRUTE)
				L.Immobilize(10)
...
/obj/structure/flora/roguegrass/bush_meagre/Initialize()
	if(silky)
		...
		if(prob(30))
			tobacco = TRUE
			berries = FALSE
			goodie = /obj/item/reagent_containers/food/snacks/grown/rogue/pipeweed
		else
			tobacco = FALSE
			berries = TRUE
			if(prob(60))
				goodie = /obj/item/reagent_containers/food/snacks/grown/berries/rogue
			else
				goodie = /obj/item/reagent_containers/food/snacks/grown/berries/rogue/poison
	pixel_x += rand(-3,3)
	if(prob(10))
...
/obj/structure/flora/roguegrass/bush_meagre/attack_hand(mob/living/user)
	...
	if(do_after(L, rand(5,20), target = src))
		...
		if(prob(prob2findstuff))
			...
			to_chat(user, "<span class='info'>Didn't find anything.</span>")
	prob2findstuff = 18
	prob2findgoodie = 15
	luckydouble	= 3
...
/obj/item/reagent_containers/food/snacks/grub/silk/attack_self(mob/living/user)
	user.visible_message("<span class='notice'>[user] crushes [src], forcing the silk out.</span>", "<span class='notice'>I crush [src], forcing the silk out.</span>")
