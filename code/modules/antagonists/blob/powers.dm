		...
		if(B)
			...
			if(cardinalblobs.len)
				...
				if(attacksuccess)
					...
					playsound(OB, 'sound/blank.ogg', 50, TRUE)
					add_points(BLOB_ATTACK_REFUND)
				else
...
/mob/camera/blob/verb/blob_help()
	...
	to_chat(src, "<b>I can expand, which will attack people, damage objects, or place a Normal Blob if the tile is clear.</b>")
	...
	to_chat(src, "<b>I can upgrade normal blobs into the following types of blob:</b>")
	to_chat(src, "<i>Shield Blobs</i> are strong and expensive blobs which take more damage. In additon, they are fireproof and can block air, use these to protect myself from station fires. Upgrading them again will result in a reflective blob, capable of reflecting most projectiles at the cost of the strong blob's extra health.")
