...
/datum/action/innate/elite_attack/singular_shot
	...
	chosen_message = "<span class='boldwarning'>I am now creating a single linear magic square.</span>"
	chosen_attack_num = SINGULAR_SHOT

/datum/action/innate/elite_attack/magic_box
	name = "Magic Box"
	button_icon_state = "magic_box"
	chosen_message = "<span class='boldwarning'>I am now attacking with a box of magic squares.</span>"
...
/mob/living/simple_animal/hostile/asteroid/elite/pandora/proc/singular_shot_line(var/procsleft, var/angleused, turf/T)
	if(procsleft <= 0)
		return
...
/mob/living/simple_animal/hostile/asteroid/elite/pandora/proc/pandora_teleport_2(var/turf/T, turf/source)
...
/mob/living/simple_animal/hostile/asteroid/elite/pandora/proc/aoe_squares_2(var/turf/T, var/ring, max_size)
	if(ring > max_size)
		return
