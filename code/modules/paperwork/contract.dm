...
/obj/item/paper/contract/employment/attack(mob/living/M, mob/living/carbon/human/user)
	...
	if(deconvert)
		...
		to_chat(M, "<span class='boldnotice'>I feel that my soul has returned to its rightful owner, Nanotrasen.</span>")
		M.return_soul()
	else
		M.visible_message("<span class='danger'>[user] beats [M] over the head with [src]!</span>", \
			"<span class='danger'>[user] beats [M] over the head with [src]!</span>")
	return ..()
...
/obj/item/paper/contract/infernal/attackby(obj/item/P, mob/living/carbon/human/user, params)
	...
	if(istype(P, /obj/item/pen) || istype(P, /obj/item/toy/crayon))
		...
		to_chat(user, "<span class='notice'>I stamp the paper with my rubber stamp, however the ink ignites as you release the stamp.</span>")
...
/obj/item/paper/contract/infernal/attack(mob/M, mob/living/user)
	...
	if(M == user && target == M.mind && M.mind.soulOwner != owner && attempt_signature(user, 1))
		user.visible_message("<span class='danger'>[user] slices [user.p_their()] wrist with [src], and scrawls [user.p_their()] name in blood.</span>", "<span class='danger'>I slice my wrist open and scrawl my name in blood.</span>")
...
/obj/item/paper/contract/infernal/proc/attempt_signature(mob/living/carbon/human/user, blood = 0)
	if(!user.IsAdvancedToolUser() || !user.is_literate())
		to_chat(user, "<span class='warning'>I don't know how to read or write!</span>")
		return 0
	if(user.mind != target)
		to_chat(user, "<span class='warning'>My signature simply slides off the sheet, it seems this contract is not meant for you to sign!</span>")
		return 0
	if(user.mind.soulOwner == owner)
		to_chat(user, "<span class='warning'>This devil already owns my soul, you may not sell it to [owner.p_them()] again!</span>")
		return 0
	if(signed)
	...
	if(HAS_TRAIT(user, TRAIT_DUMB))
		to_chat(user, "<span class='notice'>I quickly scrawl 'your name' on the contract.</span>")
		signIncorrectly()
		return 0
...
/obj/item/paper/contract/infernal/revive/attack(mob/M, mob/living/user)
	if (target == M.mind && M.stat == DEAD && M.mind.soulOwner == M.mind)
		...
		var/response = "No"
		if(ghost)
			ghost.notify_cloning("A devil has offered you revival, at the cost of my soul.",'sound/blank.ogg', H)
			response = tgalert(ghost, "A devil is offering you another chance at life, at the price of my soul, do you accept?", "Infernal Resurrection", "Yes", "No", "Never for this round", 0, 200)
			if(!ghost)
				return		//handle logouts that happen whilst the alert is waiting for a response.
		else
			response = tgalert(target.current, "A devil is offering you another chance at life, at the price of my soul, do you accept?", "Infernal Resurrection", "Yes", "No", "Never for this round", 0, 200)
		if(response == "Yes")
...
/obj/item/paper/contract/infernal/proc/fulfillContract(mob/living/carbon/human/user = target.current, blood = FALSE)
	...
	to_chat(user, "<span class='notice'>A profound emptiness washes over you as you lose ownership of my soul.</span>")
