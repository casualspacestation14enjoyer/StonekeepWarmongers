...
/obj/item/organ/vocal_cords/colossus/can_speak_with()
	...
	if(!owner)
		return FALSE
	if(!owner.can_speak())
		to_chat(owner, "<span class='warning'>I am unable to speak!</span>")
		return FALSE
	return TRUE
...
/proc/voice_of_god(message, mob/living/user, list/span_list, base_multiplier = 1, include_speaker = FALSE, message_admins = TRUE)
	...
	var/static/regex/whoareyou_words = regex("who are you|say my name|state my name|identify")
	var/static/regex/saymyname_words = regex("say my name|who am i|whoami")
	var/static/regex/knockknock_words = regex("knock knock")
	var/static/regex/statelaws_words = regex("state laws|state my laws")
