...
/obj/item/organ/zombie_infection/process()
	...
	if(!owner.getorgan(/obj/item/organ/brain))
		return
	if(!iszombie(owner))
		to_chat(owner, "<span class='cultlarge'>I can feel my heart stopping, but something isn't right... \
		life has not abandoned my broken form. You can only feel a deep and immutable hunger that \
